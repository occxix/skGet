...
async function syncHandler(
  ...
  try {
    ...
    if (errMsg.includes('REMOTE_NOT_CONFIGURED')) {
      ...
      console.error(chalk.gray('  qskills config set remote.url <git-url>'));
      console.error(chalk.gray('  qskills config set remote.branch main\n'));
    } else {
  ...
  if (result.success) {
    ...
    if (result.conflicts?.length > 0) {
      ...
      if (result.action === 'sync') {
        console.log(chalk.gray('\n  Resolve conflicts manually, then run `qskills sync` again.'));
      }
      console.log();
    }
  }
...
function outputStatus(status: any): void {
  ...
  if (!status.remoteConfigured) {
    ...
    console.log(chalk.gray('  Run: qskills config set remote.url <git-url>\n'));
    return;
  }
